...
w("- **Autos do PJe → atos → índice híbrido → recuperação citável** (espec `~/.notebooklm/ESPEC-INTELIGENCIA-JURIDICA-CRIMINAL.md`):")
w("  1. `pdf2md` perfil PJe (ou `pdf_para_markdown.py`) → `.md` paginado `## [p. N]`;")
w("  2. `fatiar_atos.py <vol01.md> [vol02.md …] --cnj <CNJ>` → um `.md` por ato processual (fronteira = `Num. X - Pág. N` + assinatura, OCR do id tolerado), `atos.jsonl`, `relatorio_fatiamento.md`;")
w("  3. `.venv-rag314/Scripts/python indexar.py casos --extracted <pasta>/extracted` → SQLite FTS5 + LanceDB/bge-m3, um chunk por página, em `%LOCALAPPDATA%\\cerebro\\casos.*` (**fora do OneDrive**; corpora públicos vão para `vault.*` com `indexar.py vault --pasta … --colecao …`);")
w("  4. `.venv-rag314/Scripts/python buscar.py casos \"<pergunta>\" --cnj <CNJ> [--tipo DECISAO] [--json]` → FTS5 ∪ vetorial → reranker `bge-reranker-v2-m3` → top-k prefixado `[ID <id_pje> | <vol> p. <n> | <tipo> | <data>]`. A minuta só cita o que veio daqui; o validador de fontes confere o prefixo.")
w("  5. distiladores sem LLM sobre o `atos.jsonl`: `cronologia_atos.py` (linha do tempo por ato + `marcos.json`: recebimento, audiência, sentença, trânsito, lidos do tipo e do texto na forma afirmativa) · `prazo_cpp.py --atos …` (radar de prazos da defesa em dias corridos, CPP 798, Súm. 310/710 STF, rito detectado na capa; `--intimacao D --dias N` calcula um prazo; `--comarca` liga feriado municipal) · `prescricao_cp.py --dispositivo lei9605:50 --fato D --atos …` (CP 109-117, pena lida do dataset do artigo, marcos do `marcos.json`, `--pena-concreta` para CP 110 §1º). Feriados em `calendario_forense.py`.")
w("  6. `distilar_atos.py atos.jsonl [--distiladores controversia,nulidades,prisao,dosimetria]` → LLM local (qwen3:8b, temperatura 0, JSON com schema do Ollama, um ato por chamada, trecho literal conferido) grava `distill/llm/*.json` e `distill/{controversia,nulidades,prisao,dosimetria}.md`; atos acima do teto vão para `distill/para_nuvem.json`. O local EXTRAI, não julga: vícios/nulidades são do agente `auditoria-nulidades-criminal`.")
w("  7. nuvem: `anonimizar.py --cnj X --capa … --denuncia … [--nomes …] [--termos …]` monta o mapa de pseudônimos (fora do OneDrive) e `llm_nuvem.py --tier forte|longo|barato|nacional --cnj X --arquivo … [--dry-run]` anonimiza → chama (tiers em `config_llm.json`, `data_collection: deny`) → reverte, logando só hash/tokens/custo em `relatorios/llm-log.jsonl`. Chaves só por variável de ambiente (`OPENROUTER_API_KEY`, `FCC_PROXY_TOKEN`, `MARITACA_API_KEY`).")
w("  8. gate pós-inferência: `validar_minuta.py minuta.md --atos <extracted>/atos.jsonl --peca resposta-acusacao|memoriais|apelacao|hc|rese|resp|re|… [--rito jecrim] [--intimacao D]` → `validador_fontes.py` (súmulas, temas de RG, artigos dos datasets, precedentes/temas por busca literal nos corpora, informativos, IDs e páginas do caso; marcador inline `[VERIFICAÇÃO NECESSÁRIA: …]`) + `validador_processual.py` (nulidade→pedido, pedido→fundamentação, rito, honorários em peça criminal, rol de testemunhas, prequestionamento/RG, tempestividade, âncoras) → `*.validada.md`, dois relatórios, `*.citacoes_para_verificar.json` para o agente `verificador-citacoes`. Só APROVADO vai ao .docx; 'não localizada' nunca é 'não existe'.")
w("  9-10. no vault Criminal: `/analisar-autos <CNJ>` e `/minutar <peça> <CNJ>` (scripts espelhados em `Criminal/.claude/tools/pipeline/`); lote: `lote_noturno.py [--sem-llm] [--janela-min N]` roda tudo para cada `%LOCALAPPDATA%\\cerebro\\casos\\<CNJ>\\caso.json` e grava `relatorios/custo-<data>.md` (tempo por etapa, tokens locais, US$ em nuvem); `LOTE_NOTURNO.bat` é o alvo do `schtasks` (não registrado por padrão).")
w("  11. interface: `rag_server.py [--porta 8765]` expõe busca, casos, arquivos do caso, gate e pseudonimização por HTTP local (127.0.0.1); `openwebui_tool_cerebro.py` é a Tool do Open WebUI que o consome (Open WebUI exige Python 3.11-3.12; ver espec §7 fase 11).")
w("  Seletores do PJe (TJMA = método `pje4`) lidos da extensão MinutaIA Conecta: `tools/pje_seletores_conecta.json`. Benchmark do LLM local: `Criminal/40-Recursos/benchmark-llm-local.md`.")
w("")
